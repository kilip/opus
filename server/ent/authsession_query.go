...
func (_q *AuthSessionQuery) QueryTokens() *AuthTokenQuery {
...
func (_q *AuthSessionQuery) WithTokens(opts ...func(*AuthTokenQuery)) *AuthSessionQuery {
	query := (&AuthTokenClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AuthSessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuthSession, error) {
	...
	if query := _q.withTokens; query != nil {
		if err := _q.loadTokens(ctx, query, nodes,
			func(n *AuthSession) { n.Edges.Tokens = []*AuthToken{} },
			func(n *AuthSession, e *AuthToken) { n.Edges.Tokens = append(n.Edges.Tokens, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AuthSessionQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*AuthSession, init func(*AuthSession), assign func(*AuthSession, *User)) error {
	...
	nodeids := make(map[string][]*AuthSession)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (_q *AuthSessionQuery) loadTokens(ctx context.Context, query *AuthTokenQuery, nodes []*AuthSession, init func(*AuthSession), assign func(*AuthSession, *AuthToken)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(authtoken.FieldSessionID)
	}
	query.Where(predicate.AuthToken(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(authsession.TokensColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
