...
export default function DashboardPage() {
  ...
  const { output, isConnected, error, clearOutput } = useStream(shouldConnect ? accessToken : null);
    ...
    <div className="container py-6 space-y-6 flex-1 flex flex-col">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Welcome back, {user?.name ?? "Explorer"}</h1>
        ...
        <div className="flex gap-2">
          {!shouldConnect ? (
            ...
            <Button variant="outline" onClick={() => setShouldConnect(false)} className="gap-2">
      ...
      <Card className="flex-1 flex flex-col border-none bg-muted/50 overflow-hidden">
        ...
        <CardContent className="flex-1 p-0 relative min-h-[400px]">
          <StreamOutput output={output} isConnected={isConnected} error={error} />
        </CardContent>
      </Card>
    </div>
  );
